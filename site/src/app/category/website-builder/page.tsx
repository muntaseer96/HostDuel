...
export default async function WebsiteBuilderCategory() {
  ...
  const hosts = allHosts
    .filter((host) => host.hostingType === 'website-builder')
    ...
    <section className="py-16">
      <Container>
        <div className="mb-12">
          ...
          <h1 className="text-3xl font-bold text-foreground mb-4">Website Builders</h1>
          <p className="text-lg text-text-secondary max-w-2xl">
            Website builders let you create professional websites without any coding knowledge.
            Drag-and-drop editors, beautiful templates, and built-in hosting make it easy
          ...
          {hosts.map((host) => (
            <Card key={host.id} className="p-6">
              ...
              <div className="space-y-2 mb-4 text-sm text-text-secondary">
                ...
                {host.freeSsl && <p className="text-success">Free SSL included</p>}
              </div>
        ...
        {hosts.length === 0 && (
          <div className="text-center py-12">
            <p className="text-text-muted">No website builders found.</p>
          </div>
        )}
